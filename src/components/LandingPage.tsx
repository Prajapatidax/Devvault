...
export const LandingPage: React.FC<LandingPageProps> = ({ onEnterApp, isWorkspaceView = false }) => {
  ...
  const services = [
    ...
    },
    {
      ...
      icon: <BookOpen className="h-5 w-5 text-indigo-450" />,
  ...
  return (
    <div className={`w-full overflow-y-auto bg-zinc-950 text-zinc-150 font-sans transition-all duration-300 relative ${isWorkspaceView ? "h-full rounded-2xl border border-zinc-800/80 p-6 bg-zinc-950/40 backdrop-blur-md" : "min-h-screen"}`}>
      {/* Glow Effects */}
      {!isWorkspaceView && (
        <>
          <div className="absolute top-0 left-1/4 -translate-x-1/2 w-[500px] h-[500px] bg-gradient-to-b from-orange-500/10 via-amber-400/5 to-transparent blur-[160px] rounded-full pointer-events-none z-0" />
          <div className="absolute bottom-1/3 right-1/4 translate-x-1/2 w-[600px] h-[600px] bg-gradient-to-t from-indigo-500/5 via-violet-500/5 to-transparent blur-[180px] rounded-full pointer-events-none z-0" />
        </>
      )}

      <div className={`max-w-6xl mx-auto z-10 relative ${isWorkspaceView ? "" : "px-6 py-12"}`}>
        {/* Landing Page Navbar */}
        {!isWorkspaceView && (
          <header className="flex items-center justify-between border-b border-zinc-900 pb-6 mb-12 select-none">
            <div className="flex items-center gap-2">
              <ArtificialLogo className="h-8 w-12 text-brand-500" />
              <div>
                <span className="font-bold text-lg tracking-tight text-white">DevVault</span>
                <span className="block text-[10px] text-zinc-500 font-mono">SECURE DEVELOPER WORKSTATION</span>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <a href="#services" className="text-xs text-zinc-400 hover:text-white transition-colors">Services</a>
              <a href="#security" className="text-xs text-zinc-400 hover:text-white transition-colors">Security</a>
              <a href="#tutorial" className="text-xs text-zinc-400 hover:text-white transition-colors">Tutorial</a>
        ...
        {/* Hero Section */}
        {!isWorkspaceView && (
          <section className="text-center py-12 md:py-20 max-w-3xl mx-auto flex flex-col items-center gap-6">
            <motion.div
              initial={{ opacity: 0, y: -20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6 }}
              className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-orange-500/30 bg-orange-500/5 text-orange-400 text-xs font-mono"
            >
            ...
            <motion.h1
              ...
              className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight leading-none bg-clip-text text-transparent bg-gradient-to-r from-white via-zinc-200 to-orange-400"
            >
            ...
            <motion.p
              ...
              className="text-base text-zinc-400 leading-relaxed font-sans max-w-2xl"
            >
        ...
        <section id="tutorial" className="py-12 border-t border-zinc-900">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-1">Watch this quick walkthrough video to master the vault workspace.</p>
            </div>
            
            {/* Show edit button only when logged in */}
            {user && (
              <Button variant="secondary" size="sm" onClick={() => setIsEditing(true)} className="text-xs">
          ...
          <div className="relative group rounded-2xl border border-zinc-800 bg-zinc-900/50 overflow-hidden shadow-2xl p-2 md:p-4 backdrop-blur-sm">
            {loadingVideo ? (
        ...
        <section id="services" className="py-16 border-t border-zinc-900">
          <div className="text-center max-w-xl mx-auto mb-12">
            <h2 className="text-2xl md:text-3xl font-bold text-white">Services We Provide</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((srv, idx) => (
              <div
                key={idx}
                className={`p-6 rounded-xl border bg-gradient-to-br ${srv.color} bg-zinc-900/10 hover:bg-zinc-900/40 transition-all duration-300 flex flex-col gap-3 group`}
              >
                <div className="p-2.5 rounded-lg bg-zinc-950 w-fit border border-zinc-800 group-hover:border-zinc-700/85 transition-colors">
                  {srv.icon}
                </div>
        ...
        <section className="py-16 border-t border-zinc-900">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">What Details Can You Share?</h2>
              ...
              <div className="flex flex-col gap-4">
                {shareableDetails.map((detail, idx) => (
                  <div key={idx} className="flex gap-3">
                    <div className="h-5 w-5 rounded-full bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-400 text-[10px] font-mono shrink-0 mt-0.5 font-bold">
                      {idx + 1}
                    </div>
                    <div>
                      <h4 className="font-bold text-sm text-white">{detail.title}</h4>
                      <p className="text-xs text-zinc-400 mt-1 leading-relaxed">{detail.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-zinc-900/30 border border-zinc-850 p-6 rounded-2xl relative overflow-hidden backdrop-blur-md">
              ...
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="font-mono text-xs font-bold text-zinc-400 uppercase tracking-wider">COLLABORATIVE SHARING MATRIX</h3>
              </div>

              <div className="border border-zinc-800 rounded-lg overflow-hidden text-xs">
                <div className="grid grid-cols-3 bg-zinc-950 p-3 font-mono text-zinc-500 font-bold border-b border-zinc-800">
                  <span>DATA TYPE</span>
                  <span>VISIBILITY</span>
                  <span>ENCRYPTION</span>
                </div>
                <div className="grid grid-cols-3 p-3 border-b border-zinc-900 hover:bg-zinc-900/10 transition-colors">
                  <span className="font-semibold text-white">Project Boards</span>
                  <span className="text-amber-400 font-medium">Invited Members</span>
                  <span className="text-zinc-500 font-mono">SSL Transport</span>
                </div>
                <div className="grid grid-cols-3 p-3 border-b border-zinc-900 hover:bg-zinc-900/10 transition-colors">
                  <span className="font-semibold text-white">Project Secrets</span>
                  <span className="text-amber-400 font-medium">Role Restricted</span>
                  <span className="text-emerald-400 font-mono">AES-256 encrypted</span>
                </div>
                <div className="grid grid-cols-3 p-3 border-b border-zinc-900 hover:bg-zinc-900/10 transition-colors">
                  <span className="font-semibold text-white">Markdown Notes</span>
                  <span className="text-amber-400 font-medium">Project Scope</span>
                  <span className="text-zinc-500 font-mono">SSL Transport</span>
                </div>
                <div className="grid grid-cols-3 p-3 hover:bg-zinc-900/10 transition-colors">
                  <span className="font-semibold text-white">Bug Reports</span>
                  <span className="text-emerald-400 font-medium">Public/Project</span>
        ...
        {/* Security Footprint */}
        <section id="security" className="py-16 border-t border-zinc-900">
          <div className="text-center max-w-xl mx-auto mb-12">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-emerald-500/30 bg-emerald-500/5 text-emerald-400 text-xs font-mono mb-3">
              <ShieldCheck className="h-3.5 w-3.5" />
              <span>Ironclad Security Layer</span>
            </div>
            <h2 className="text-2xl md:text-3xl font-bold text-white">How Much Security We Have?</h2>
            <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {securityFootprint.map((sec, idx) => (
              <div key={idx} className="flex gap-4 p-5 rounded-xl border border-zinc-900 bg-zinc-950/20 hover:border-zinc-800 transition-all duration-300">
                <div className="p-3 rounded-lg bg-zinc-900 border border-zinc-800 text-white w-fit h-fit shrink-0">
                  {sec.icon}
                </div>
                <div>
                  <h3 className="font-bold text-sm text-white">{sec.title}</h3>
                  <p className="text-xs text-zinc-400 mt-1.5 leading-relaxed">{sec.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Footer */}
        {!isWorkspaceView && (
          <footer className="border-t border-zinc-900 pt-10 mt-20 text-center flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-zinc-550 select-none">
            <div className="flex items-center gap-2">
              <ArtificialLogo className="h-6 w-10 text-brand-500" />
              <span className="font-bold text-white">DevVault</span>
            ...
            <div className="flex items-center gap-4 text-zinc-400">
              ...
              <span>•</span>
              <span className="hover:text-white transition-colors cursor-pointer" onClick={onEnterApp}>Enter Workspace</span>
            </div>
          </footer>
        )}
      </div>
        ...
        {isEditing && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6 relative z-10 shadow-2xl"
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-white text-base">Update Tutorial Video</h3>
                <button
                  onClick={() => setIsEditing(false)}
